  ...
  app.innerHTML = page(`
    <section class="shell hero">
      <div class="hero-copy">
        ...
        <ul class="facts"><li>Proof links expire after 14 days.</li><li>Clients open links without an account.</li><li>$59 per business each month.</li></ul>
      </div>
      <div class="hero-art">
    ...
    <section class="section" id="pricing" aria-labelledby="price-title"><div class="shell"><div class="price-sheet">
      <div><p class="eyebrow">Business plan</p><h2 id="price-title">Keep every recurring visit in the loop</h2><p>Includes unlimited client proof links, extra choices, and next-visit CSV exports.</p><ul class="plain-list"><li>One business workspace</li><li>Technician seats added as your team grows</li><li>Cancel before the next billing month</li></ul></div>
      <div><p class="price">$59 <small>per business / month</small></p><a class="button" href="${BILLING}/checkout">Buy the business plan <span class="sr-only">at Sociobot checkout</span></a>
...
async function manageExtras(mode: 'demo'|'real') {
  ...
  try {
    ...
    document.querySelector<HTMLFormElement>('#extra-form')?.addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const values = new FormData(form); const button = form.querySelector('button')!; button.disabled = true; button.textContent = 'Adding choice…'; try { await api('/extras', {method:'POST',headers:jsonHeaders,body:JSON.stringify({name:values.get('name'),detail:values.get('detail'),price_cents:Math.round(Number(values.get('price'))*100)})}, mode); toast('Client choice added.'); await manageExtras(mode); } catch(error) { showError(error); button.disabled = false; button.textContent = 'Add client choice'; } });
...
function onboarding() {
  ...
  document.querySelector<HTMLFormElement>('#workspace-form')?.addEventListener('submit', async event => {
    event.preventDefault(); const form = event.currentTarget; const button = form.querySelector('button')!; button.disabled = true; button.textContent = 'Creating workspace…';
...
function legal(kind: 'privacy'|'terms') {
  ...
  app.innerHTML = page(`<article class="shell legal measure"><p class="eyebrow">Last updated August 28, 2026</p><h1>${privacy ? 'Privacy for visit proof' : 'Terms of service'}</h1>
    ${privacy ? `<p>Service Proof Loop stores the details needed to share completed work and plan the next visit.</p><h2>What we store</h2><p>We store business names, client labels, visit notes, consented photos, ratings, comments, and chosen extras.</p><p>Do not enter door codes, payment cards, health records, or other unnecessary private details.</p><h2>Why we store it</h2><p>We use this data only to deliver proof links, collect replies, and prepare next-visit exports.</p><h2>Proof links</h2><p>Each proof link uses a random token and expires after 14 days. Anyone with the link can view its visit.</p><h2>Demo data</h2><p>Each demo uses an isolated workspace. It expires within 24 hours and never opens a real workspace.</p><h2>Billing</h2><p>Sociobot and Dodo handle checkout, licenses, and refunds. This service does not receive payment card numbers.</p><h2>Your choices</h2><p>Ask the business that sent your link to correct or remove visit data. Businesses can contact support for account deletion.</p><h2>Contact</h2><p>Email <a href="mailto:privacy@sociobot.in">privacy@sociobot.in</a> with privacy questions.</p>` : `<p>These terms apply when you use Service Proof Loop.</p><h2>Use the service fairly</h2><p>Use the service for lawful visit proof and client feedback. Get consent before taking or sharing photos.</p><p>Do not upload harmful content, access another workspace, or test proof links you did not receive.</p><h2>Your responsibilities</h2><p>You control what your team records. Keep workspace access keys on devices your business controls.</p><h2>Subscription</h2><p>The business plan costs $59 per business each month. Sociobot is the merchant of record.</p><p>You may cancel before the next billing month. Refunds follow the checkout terms and revoke the related license.</p><h2>Service limits</h2><p>This product does not provide dispatch, payments, payroll, worker tracking, or emergency communication.</p><h2>Availability</h2><p>We work to keep the service available. We cannot promise uninterrupted access or permanent storage.</p><h2>Contact</h2><p>Email <a href="mailto:support@sociobot.in">support@sociobot.in</a> with service questions.</p>`}
  </article>`);
}
...
function bindLicense() {
  document.querySelector<HTMLFormElement>('#license-form')?.addEventListener('submit', async event => {
    event.preventDefault(); const token = String(new FormData(event.currentTarget).get('license') || '').trim(); if (!token) return;
