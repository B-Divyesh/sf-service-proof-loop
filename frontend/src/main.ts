...
async function manageExtras(mode: 'demo'|'real', currentExtras?: Extra[]) {
  ...
  try {
    ...
    document.querySelector<HTMLFormElement>('#extra-form')?.addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const values = new FormData(form); const button = form.querySelector<HTMLButtonElement>('button')!; button.disabled = true; button.textContent = 'Adding choice…'; try { const created = await api<Extra>('/extras', {method:'POST',headers:jsonHeaders,body:JSON.stringify({name:values.get('name'),detail:values.get('detail'),price_cents:Math.round(Number(values.get('price'))*100)})}, mode); toast('Client choice added.'); await manageExtras(mode, [...extras, created]); } catch(error) { showError(error); button.disabled = false; button.textContent = 'Add client choice'; } });
