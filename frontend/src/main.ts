  ...
  app.innerHTML = page(`
    <section class="shell hero">
      <div class="hero-copy">
        ...
        <p class="lede">For recurring service teams that need client feedback and approved extras without asking clients to install an app.</p>
        ...
        <ul class="facts"><li>Proof links expire after 14 days.</li><li>Clients open links without an account.</li><li>Three visits free. $59 once for one workspace.</li></ul>
      </div>
      <div class="hero-art">
        ...
        <div class="art-note">An approved extra appears on the next visit.</div>
      </div>
    </section>
    <section class="section" aria-labelledby="preview-title"><div class="shell">
      <div class="section-head"><p class="eyebrow">The product</p><div><h2 id="preview-title">See proof and next-visit extras</h2><p>Crews record the visit. Clients review the proof. The office exports approved extras without typing them again.</p></div></div>
    ...
    <section class="section" aria-labelledby="how-title"><div class="shell">
      <div class="section-head"><p class="eyebrow">How it works</p><div><h2 id="how-title">How proof reaches the next visit</h2></div></div>
      <div class="steps"><article><h3>Record the visit</h3><p>The technician checks the work, adds photos with consent, and sends one private link.</p></article><article><h3>Collect one clear reply</h3><p>The client accepts the work or reports a problem. They can rate it and choose extras.</p></article><article><h3>Add approved extras to the next visit</h3><p>Export the next date, client, location, extra, detail, and price as CSV.</p></article></div>
    </div></section>
    <section class="section privacy-band" aria-labelledby="limits-title"><div class="shell">
      <div class="section-head"><p class="eyebrow">Clear boundaries</p><div><h2 id="limits-title">What this service handles</h2><p>It stores visit proof, client replies, and approved extras.</p></div></div>
      <div class="limits"><div><h3>What it handles</h3><ul class="plain-list"><li>Visit checklists and consented photos</li><li>Client acceptance, problems, and ratings</li><li>Approved extras for the next visit</li></ul></div><div><h3>What it leaves alone</h3><ul class="plain-list"><li>No dispatch, payroll, or worker tracking</li><li>Do not enter home-entry codes or payment card details</li><li>No public review campaigns</li></ul></div></div>
    </div></section>
    <section class="section" id="pricing" aria-labelledby="price-title"><div class="shell"><div class="price-sheet">
      <div><p class="eyebrow">Business license</p><h2 id="price-title">Keep creating proof links</h2><p>After three free visits, one $59 license covers one business workspace.</p><ul class="plain-list"><li>More visits in that workspace</li><li>Configurable client extras</li><li>Next-visit CSV exports</li></ul></div>
...
async function workspace(mode: 'demo'|'real') {
  ...
  setMeta(`${mode === 'demo' ? 'Demo' : 'Workspace'} — Service Proof Loop`, mode === 'demo' ? 'Try a complete proof-to-next-visit loop with isolated sample data.' : 'Record completed visits and carry client extras into the next visit.');
...
function renderWorkspace(mode: 'demo'|'real', visits: Visit[], selected = 0) {
  ...
  app.innerHTML = page(`<div class="shell app-main">
    ...
    ${mode === 'real' && visits.length >= 3 && !licenseActive() ? '<p class="message">The free plan includes three visits. Add a license for more visits in this workspace.</p>':''}
...
async function manageExtras(mode: 'demo'|'real', currentExtras?: Extra[]) {
  ...
  try {
    const extras = currentExtras ?? await api<Extra[]>('/extras', {}, mode);
    app.innerHTML = page(`<section class="shell app-main"><p class="eyebrow">Extras</p><h1>Manage next-visit extras</h1><p class="lede">These extras appear on every client proof page.</p><div class="workspace-grid"><div><button class="ghost" id="back-visits">Back to visits</button></div><div><div class="panel"><h2>Current extras</h2><div class="extras">${extras.map(extra => `<div class="task-chip"><span><strong>${esc(extra.name)}</strong><br><small>${esc(extra.detail)}</small></span><strong>${money(extra.price_cents)}</strong></div>`).join('')}</div></div><form class="panel" id="extra-form"><h2>Add an extra</h2><div class="form-grid"><div class="field"><label for="extra-name">Extra name</label><input id="extra-name" name="name" maxlength="80" required></div><div class="field"><label for="extra-price">Price in dollars</label><input id="extra-price" name="price" type="number" min="0" max="1000" step="0.01" required></div><div class="field full"><label for="extra-detail">What the technician will do</label><input id="extra-detail" name="detail" maxlength="160" required></div></div><p id="form-error" aria-live="assertive"></p><div class="form-actions"><button type="submit">Add extra</button></div></form></div></div></section>`, mode === 'demo');
    bindDemoBanner(); document.querySelector('#back-visits')?.addEventListener('click', () => workspace(mode));
    document.querySelector<HTMLFormElement>('#extra-form')?.addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const values = new FormData(form); const button = form.querySelector<HTMLButtonElement>('button')!; button.disabled = true; button.textContent = 'Adding extra…'; try { const created = await api<Extra>('/extras', {method:'POST',headers:jsonHeaders,body:JSON.stringify({name:values.get('name'),detail:values.get('detail'),price_cents:Math.round(Number(values.get('price'))*100)})}, mode); toast('Extra added.'); await manageExtras(mode, [...extras, created]); } catch(error) { showError(error); button.disabled = false; button.textContent = 'Add extra'; } });
...
function setMeta(title: string, description: string) {
  ...
  const tokenBearingProof = location.pathname.startsWith('/proof/');
  ...
  if (tokenBearingProof) { canonical?.remove(); if (!robots) { const meta = document.createElement('meta'); meta.name = 'robots'; meta.content = 'noindex, nofollow, noarchive'; document.head.append(meta); } }
  else { if (robots) robots.remove(); const link = canonical || document.head.appendChild(document.createElement('link')); link.rel = 'canonical'; link.href = `https://service-proof-loop.sociobot.in${location.pathname}`; const ogUrl = document.querySelector<HTMLMetaElement>('meta[property="og:url"]'); if (ogUrl) ogUrl.content = link.href; }
}
...
function bindLicense() {
  ...
  else if (token && licenseActive()) setLicenseState('active', 'License verified. It covers one business workspace.');
...
async function verifyLicense(token: string, announce: boolean) {
  ...
  try {
    ...
    setLicenseState(result.valid ? 'active' : 'inactive', result.valid ? 'License verified. It covers one business workspace.' : 'License no longer active. Check the token or buy the plan.');
  } catch {
